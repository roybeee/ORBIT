...
한 응답에서 읽기 요청 1~32개를 받아 순서대로 처리한다. 바깥 설명/코드 블록에 둘러싸인 단일 JSON 객체는 읽되, 중복 객체·잘린 JSON·알 수 없는 도구·추가 실행 필드는 그대로 거부한다. notes가 생략되면 이전 작업 메모를 보존한다.

형식 재요청은 직전 원문 입력·메모·스키마 오류·실패 응답을 함께 전달하며, 정상 응답 후 연속 실패 횟수를 초기화한다. 연속 실패 시 최대 세 번 자동 재요청하고 이후에는 자료를 보존한 채 멈춘다. 실패 카드의 **저장된 자료로 이어서 분석**은 같은 작업 ID, 출처, 원래 지시로 다음 분석 단계를 만든다. 이전 버전에서 형식 오류로 종료된 연구 작업도 지원한다. 기존 원문을 다시 확보하려면 저장된 read_result를 사용할 수 있다. 개발 실행이나 취소·완료 작업을 이 버튼으로 재실행하지 않는다.
