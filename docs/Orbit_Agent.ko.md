...
| Google Calendar | 최초 OAuth 클라이언트 설정 → Google 연결·승인 | 캘린더 조회·동기화와 ORBIT에서 저장한 일정 자동 등록·수정 |
...
- Google을 연결하면 새로 저장·수정한 ORBIT 일정과 새로 승인한 집중 시간이 Google 기본 캘린더에도 자동 반영됩니다. 전송 대기는 ORBIT 저장과 함께 기록되며, 응답이 유실되어도 같은 일정 번호로 확인해 중복 생성을 방지합니다. 앱의 일정 새로고침과 기존 자동 동기화가 실패한 전송을 재시도합니다.
- 일정 상세에서 Google 등록 완료·반영 중·확인 필요 상태와 원본 링크를 확인합니다. 과거에 저장한 모든 일정이 일괄 업로드되지는 않습니다. 기존 일정은 수정 저장하면 등록됩니다.
- Google에서 가져온 일정은 Google Calendar에서 수정합니다. ORBIT에 등록한 일정이 Google에서 별도로 변경되면 덮어쓰지 않고 확인 필요 상태를 표시합니다. ORBIT 일정 삭제·집중 시간 승인 취소는 Google 삭제와 별개입니다.
