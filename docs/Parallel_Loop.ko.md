...
| 두 브랜치가 같은 번호의 마이그레이션 생성 | `scripts/check-migrations.mjs`가 PR과 main 푸시마다 journal 순번, 파일, 스냅샷 체인, 기준 브랜치 대비 재번호 여부를 검사 |
| `CHANGELOG.md` 동시 편집 충돌 | `.gitattributes`의 `merge=union`으로 양쪽 섹션을 모두 보존 (로컬 머지에만 적용, 아래 한계 참조) |
...
- PR head의 `Validate Orbit`이 성공할 때까지 기다린 뒤에만 `gh pr merge --auto --merge`를 켠다(필수 검사가 없는 상태에서 `--auto`는 즉시 머지되므로).
- 머지될 때까지 기다리며 `BEHIND`면 **로컬에서** `main`을 병합해 push한다. `gh pr update-branch`는 GitHub 쪽 PR head만 앞세워 로컬 브랜치와 갈라지게 하고, 그 워크트리의 다음 push가 non-fast-forward로 거부되기 때문이다. `DIRTY`(충돌)면 `sync.sh`를 안내하며 종료한다. 끝나면 새 `main` SHA와 tree를 출력한다.
...
`origin/main`에 머지된 브랜치의 워크트리와 로컬·원격 브랜치를 제거한다. 다음 중 하나라도 해당하면 `--force` 없이는 건너뛴다: 미커밋 변경, `main`에 없는 커밋, **브랜치가 아직 `main`과 같은 자리(= 자기 커밋이 없음)**, 머지된 PR 기록 없음. 마지막 두 조건이 방금 `start.sh`를 실행한 다른 에이전트의 워크트리를 보호한다.
...
- Sites publication(버전 저장·배포 ID 생성)은 ChatGPT/Codex 안의 Sites 연결로만 가능하다. 이 스크립트는 로컬 Codex CLI(`codex exec`, bundled `sites-hosting` 스킬)에 정확한 지시문을 전달한다: 검증된 SHA의 독립 클론에서 exact-tree projection push → 빌드·패키지 → `save_version_and_deploy_private` → 배포 상태 폴링 → 구조화된 보고.
- 게시용 체크아웃은 워크트리가 아니라 **독립 클론**이다. Codex의 `workspace-write` 샌드박스는 체크아웃 밖(워크트리의 `.git`)에 쓸 수 없고, Sites 원격에 닿으려면 네트워크 허용도 필요하다. 매 실행마다 체크아웃의 HEAD가 그 SHA인지, 수정이 없는지 확인하며, 게시 후 변경이 생겼으면 경고한다.
- Codex가 승인을 요청하므로 터미널에서 소유자가 직접 실행한다(무인 전권 실행은 하지 않는다). `--print`로 지시문과 명령만 확인할 수 있다.
- 끝나면 보고의 DEPLOYMENT_ID·버전을 릴리스 기록에 적고 `verify-deploy.sh`로 운영 tree를 대조한다. 게시 클론은 `node_modules` 때문에 1GB 가까이 되므로 기록을 마친 뒤 삭제한다.

### 7. 배포 검증 — `scripts/parallel/verify-deploy.sh <github-sha> [--url ...]`

`ORBIT_RELEASE_HEALTH_TOKEN`으로 `/api/deployment-health`를 호출해 보고된 `tree`가 GitHub SHA의 tree와 같은지 확인한다. 같을 때만 "운영 반영"으로 보고한다. `unknown`이면 이 변경 이전 빌드이거나, git 없는 환경에서 빌드했거나, **빌드 디렉터리가 더러운 상태**였다는 뜻이다(거짓 검증 대신 실패). `ORBIT_SOURCE_TREE`로 주입할 수 있지만 그 값은 검증되지 않으므로 신뢰할 근거가 아니다.
...
1. 머지 = `gh pr merge --auto --merge`. REST merge 직접 호출 금지. 검사받은 head가 최신 `main`을 포함해야 실제로 머지된다.
