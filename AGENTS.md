...
- One task = one worktree from the verified remote `main`. The helper scripts in `scripts/parallel/` implement this loop: `start.sh <slug>` (verified base + worktree + install), `sync.sh` (integrate the latest `main`, re-run checks), `finish.sh` (push, PR with base SHA and tree, auto-merge, re-sync when behind, wait for the merged SHA), `status.sh` (drift/PR/CI board), `cleanup.sh`, `release.sh` and `verify-deploy.sh`. See `docs/Parallel_Loop.ko.md`.
