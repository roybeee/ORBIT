...
- `publish-sites.sh` runs `scripts/parallel/preflight-quota.mjs` first. By owner decision (2026-09-23) Codex and Hermes stay on one OpenAI account: the shared account only warns that a publication spends the Hermes weekly quota, so schedule publications away from Hermes planning. It blocks when a Hermes credential has a quota error (429/quota) with a future or missing reset time; a stored `relogin_required` flag only warns. `--accept-shared-quota` only silences the shared-account hint. After a publication, the Codex report is archived in `docs/releases/publish-reports/`; `cleanup.sh --publish-clones` removes leftover publish clones whose only change is that report.
