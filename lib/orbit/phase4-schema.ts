...
export const experimentSchema=z.object({id:z.string().min(1).max(80),title:text(160),projectId:id,noteId:id,noteRevision:z.number().int().positive(),hypothesis:text(2000),action:text(2000),metric:text(100),unit:text(30),baseline:z.number().finite(),target:z.number().finite(),direction:z.enum(['up','down']),from:date,through:date,minutes:z.number().int().min(5).max(480)}).strict();
export const storedExperimentSchema=experimentSchema.extend({taskId:id,status:z.enum(['active','completed','stopped']),createdAt:z.string().datetime(),result:z.object({value:z.number().finite(),evidence:text(2000),conclusion:text(2000),at:z.string().datetime(),met:z.boolean()}).optional()});
