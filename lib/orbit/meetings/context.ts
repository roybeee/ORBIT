...
export async function recordContext(db:Database,owner:string,query:string,projectId?:string|null){
 const terms=queryTerms(query),found=new Map<string,Note&{searchExcerpt?:string}>();
 ...
 const feedbackRows=await db.prepare('SELECT f.* FROM orbit_answer_feedback f JOIN orbit_agent_turns t ON t.owner_id=f.owner_id AND t.id=f.turn_id WHERE f.owner_id=? ORDER BY f.updated_at DESC LIMIT 100').bind(owner).all<{turn_id:string;kind:string;text:string;question:string;project_id:string|null;updated_at:string}>();
