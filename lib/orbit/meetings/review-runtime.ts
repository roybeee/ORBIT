...
export async function advanceMeetingReviews(db:Database,owner:string,env?:Runtime,noteId?:string){
 // Reconcile completed/failed turns without duplicating their approval cards. A turn that failed on a
 // provider limit keeps its record as waiting work for the recovery below.
 ...
 const hold=await activeHold(db,owner,meetingProvider(env));
 if(hold){
  await db.prepare("UPDATE orbit_meeting_reviews SET hold_id=? WHERE owner_id=? AND status IN ('queued','waiting_quota') AND hold_id<>?").bind(hold.id,owner,hold.id).run();
  if(Date.now()<hold.nextCheckAt||hold.probeUntil>Date.now())return {active:false};
 }
 const row=await db.prepare("SELECT * FROM orbit_meeting_reviews WHERE owner_id=? AND status IN ('queued','waiting_quota') AND (? IS NULL OR note_id=?) ORDER BY status='waiting_quota' DESC,(SELECT json_extract(n.note_json,'$.updated') FROM orbit_note_revisions n WHERE n.owner_id=orbit_meeting_reviews.owner_id AND n.note_id=orbit_meeting_reviews.note_id AND n.revision=orbit_meeting_reviews.revision) DESC,created_at DESC LIMIT 1").bind(owner,noteId??null,noteId??null).first<{note_id:string;revision:number;turn_id:string;conversation_id:string}>();
...
export async function processMeetingReviews(db:Database,owner:string,env:Runtime,noteId?:string){
 ...
 // Records that failed on a provider limit before waiting existed are revived once, if recent.
 const failed=await db.prepare("SELECT note_id,revision,error FROM orbit_meeting_reviews WHERE owner_id=? AND status='failed' AND updated_at>=?").bind(owner,new Date(Date.now()-7*86400000).toISOString()).all<{note_id:string;revision:number;error:string}>();
 for(const r of failed.results.filter(r=>waitsForLimit(r.error)))await db.prepare("UPDATE orbit_meeting_reviews SET status='waiting_quota' WHERE owner_id=? AND note_id=? AND revision=? AND status='failed'").bind(owner,r.note_id,r.revision).run();
