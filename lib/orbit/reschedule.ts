...
import {activeAllocation,protectedEvents,portfolioBasis} from './phase3.ts';
...
export function prepareReplan(data:WorkspaceData,date:string,now=new Date()):NonNullable<Proposal['replan']>{
 ...
 const alternatives=(['normal','low'] as const).map(energy=>{
  const p=generateProposal(pool,events,date,energy,previous,data.preferences,{context:data,earliestStart,dominoProjectId:data.dominoProjectId,projectPriority:Object.fromEntries((activeAllocation(data,date)?.allocations??[]).map(a=>[a.projectId,a.stance==='focus'?50:0])),calibration:(t:Task)=>calibrationFactor(data.tasks,t,date)});
