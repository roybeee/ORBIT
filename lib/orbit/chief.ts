...
export function chiefOfStaff(data: WorkspaceData, now = new Date()) {
  ...
  if (withinWork && !protectedNow && !active && !resting && !recovery && overloaded) push({key:`load:${today}`,kind:'followup',title:'오늘의 양이 실제 여유 시간을 넘었습니다',reason:`오늘 마감·집중 작업 ${demand}분 / 지금부터 일정·돌봄·여유분을 제외한 업무 예산 ${capacity}분.`,next:'더 밀어붙이기 전에 범위 축소·위임·보류안을 정합니다.',ask:'목표에 직접 기여하는 순서로 오늘의 과부하를 해소해 줘. 실제 일정과 돌봄 시간을 지키고, 반드시 할 것 1개, 줄일 것, 미룰 것, 위임할 것을 승인 가능한 제안으로 만들어 줘.'});
  ...
  return {today,minute,energy:energy??'unknown',checkin,quiet,paused,meeting:meeting?.title,attention:!quiet&&!paused&&!meeting&&!active&&!protectedNow,capacity,demand,overloaded,wins,goals,primary:signals[0]??fallback,alternatives:signals.slice(1,3),signals,care:careEvents(data,today),snapshotAt:now.toISOString()};
}
