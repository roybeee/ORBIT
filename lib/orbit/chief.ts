import {goalIsActive,goalAllowsWork,workEligibility} from './work-policy.ts';
import {allocationAllowsWork,protectedEvents,planningEvents} from './allocation-policy.ts';
import type { CalendarEvent, Goal, WorkspaceData } from './model.ts';
import {durationText} from './model.ts';
...
export {goalIsActive,goalAllowsWork} from './work-policy.ts';
...
export function chiefOfStaff(data: WorkspaceData, now = new Date()) {
  ...
  const intervals = planningEvents([...data.events.filter(e => e.date === today), ...careEvents(data,today),...protectedEvents(data,today)],today,data.preferences).filter(e=>e.date===today).map(e => [Math.max(from,e.start), Math.min(data.preferences.workEnd,e.end)]).filter(([a,b]) => b > a).sort((a,b) => a[0]-b[0]);
  ...
  const demandTasks = ready.filter(t => !reserved.has(t.id) && (t.focusDate === today || t.due <= today));
  const demand = demandTasks.reduce((s,t)=>s+t.duration,0);
  const windowMinutes = workDay ? data.preferences.workEnd-from : 0;
  const occupiedMinutes = workDay ? occupied : 0;
  const timeBudget = {
    phase: !workDay ? 'off' as const : minute >= data.preferences.workEnd ? 'ended' as const : minute < data.preferences.workStart ? 'before' as const : 'open' as const,
    start: from, end: data.preferences.workEnd, windowMinutes, occupiedMinutes,
    bufferMinutes: windowMinutes-occupiedMinutes-capacity,
    tasks: demandTasks.map(t=>({id:t.id,title:t.title,minutes:t.duration,overdue:t.due<today})),
  };
  ...
  else if (protectedNow) signals.push({key:'protected:'+protectedNow.id,kind:'rest',title:'지금은 보호한 시간입니다',reason:protectedNow.title,next:'가족·건강·회복을 위해 비워 둔 시간을 지킵니다.',ask:'주간 배분에서 보호한 시간을 지키며 지금은 새 업무를 더하지 마.'});
  ...
  if (withinWork && !protectedNow && !active && !resting && !recovery && overloaded) push({key:`load:${today}`,kind:'followup',title:'남은 시간보다 할 일이 많아요',reason:`미배치 할 일 예상 ${durationText(demand)} · 쓸 수 있는 시간 ${durationText(capacity)} · ${durationText(demand-capacity)} 초과`,next:'오늘 할 일의 범위를 줄이거나, 미룰 일과 위임할 일을 골라보세요.',ask:'목표에 직접 기여하는 순서로 오늘의 과부하를 해소해 줘. 실제 일정과 돌봄 시간을 지키고, 반드시 할 것 1개, 줄일 것, 미룰 것, 위임할 것을 승인 가능한 제안으로 만들어 줘.'});
  if (!active && !resting && !protectedNow) {
  ...
  return {today,minute,energy:energy??'unknown',checkin,quiet,paused,meeting:meeting?.title,attention:!quiet&&!paused&&!meeting&&!active&&!protectedNow,capacity,demand,timeBudget,overloaded,wins,goals,primary:signals[0]??fallback,alternatives:signals.slice(1,3),signals,care:careEvents(data,today),snapshotAt:now.toISOString()};
}
