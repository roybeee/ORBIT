...
export const asideInput = z.discriminatedUnion('action', [
  z.object({action:z.literal('enqueue'),id:uuid,title:z.string().trim().min(1).max(160),instruction:z.string().trim().min(10).max(12000),workflow:z.string().max(50),projectId:z.string().max(100)}).strict(),
...
export async function listAsideJobs(db:Database,ownerId:string) {
  const rows=await db.prepare("SELECT job_json FROM orbit_aside_jobs WHERE owner_id=? ORDER BY CASE WHEN status IN ('running','stop_requested','needs_attention','queued') THEN 0 ELSE 1 END, created_at DESC LIMIT 100").bind(ownerId).all<Row>();
...
export async function changeAsideJob(db:Database,ownerId:string,input:Input):Promise<AsideJob> {
  ...
  if(input.action==='enqueue') {
    ...
    await db.prepare("INSERT INTO orbit_aside_jobs (owner_id,id,status,job_json,version,created_at) SELECT ?,?,'queued',?,0,? WHERE (SELECT COUNT(*) FROM orbit_aside_jobs WHERE owner_id=? AND status='queued')<30 ON CONFLICT(owner_id,id) DO NOTHING").bind(ownerId,job.id,JSON.stringify(job),now,ownerId).run();
    ...
    if(['title','instruction','workflow','projectId'].some(key=>saved[key as keyof AsideJob]!==job[key as keyof AsideJob]))throw new AgentError('같은 작업 ID의 내용이 다릅니다.','CONFLICT',409);
    return saved;
  }
  for(let attempt=0;attempt<5;attempt++) {
    ...
    if(input.action==='claim') {
      ...
      if(old.status==='queued'){job.status='cancelled';job.progress='실행 전에 종료했습니다.';}
