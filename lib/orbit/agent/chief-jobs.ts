...
export async function chiefScheduleStatus(db:Database,owner:string,env:Runtime){const config=await hermesConfig(db,owner,env),state=await receiptFor(db,owner,config.connectionId);const row=await db.prepare('SELECT config_json FROM orbit_chief_jobs WHERE owner_id=?').bind(owner).first<{config_json:string}>();return {...publicJob(await findJob(config,await chiefJobName(owner,config.connectionId)),state),restored:row?JSON.parse(row.config_json).restored??null:null};}
