...
async function session(db:Database,owner:string,env:Runtime){
 ...
 async function rpc(method:string,params:unknown,notification=false):Promise<Record<string,unknown>>{
  ...
  try{
   ...
   const result=(message:{jsonrpc?:unknown;id?:unknown;error?:unknown;result?:Record<string,unknown>})=>{if(message?.jsonrpc!=='2.0'||message.id!==id)return undefined;if(message.error||!('result' in message))throw new AgentError('Plaud가 기록을 반환하지 못했습니다. 녹음 범위와 접근 권한을 확인해 주세요.','PLAUD_MCP',502);return message.result};
   try{
 ...
 const initialized=await rpc('initialize',{protocolVersion:version,capabilities:{},clientInfo:{name:'Orbit',version:'0.5.0'}}) as {protocolVersion:string};
 ...
 async function list(){let cursor:string|undefined;const catalog:McpTool[]=[];for(let page=0;page<5;page++){const data=await rpc('tools/list',cursor?{cursor}:{}) as {tools?:unknown;nextCursor?:string};if(!Array.isArray(data.tools)||data.tools.length>100)throw new AgentError('Plaud 조회 도구를 확인하지 못했습니다.','PLAUD_MCP',502);catalog.push(...data.tools.filter((t:McpTool)=>typeof t.name==='string'&&t.inputSchema&&isRead(t)));if(!data.nextCursor)return catalog;cursor=data.nextCursor}throw new AgentError('Plaud 조회 도구 목록이 너무 큽니다.','PLAUD_MCP',502)}
 return {rpc,list};
}
...
async function plaudReadInternal(db:Database,owner:string,env:Runtime,name:string,args:Record<string,unknown>){
 ...
 const data=await client.rpc('tools/call',{name,arguments:args}) as {isError?:unknown;content?:{type?:unknown;text?:unknown}[];structuredContent?:unknown};
 ...
 return {content:(data.content??[]).filter(c=>c.type==='text').map(c=>({type:'text',text:c.text})),...(data.structuredContent?{structuredContent:data.structuredContent}:{})};
}
