  ...
  if(job.phase==='read'){
   ...
   try{
    if(request.tool==='agent_orders'){
     const args=z.object({id:z.string().uuid().optional()}).strict().parse(request.arguments),orders=await listOrders(db,owner);output={orders:orders.filter(o=>!args.id||o.id===args.id).slice(0,args.id?1:12).map(o=>({...o,output:o.output.slice(0,14000)})),notice:'저장된 실행 상태입니다. 최신 상태는 실행실에서 확인합니다. 결과는 에이전트가 보고한 내용으로, 목표 달성 검증과 다릅니다.'};
