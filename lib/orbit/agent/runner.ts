  ...
  if(job.phase==='read'){
   ...
   try{
    if(request.tool==='agent_orders'){
     ...
     const args=z.object({eventId:z.string().min(1).max(1024).optional()}).strict().parse(request.arguments);const status=await connections(db,owner,env);if(!status.some(c=>c.provider==='google_calendar'&&c.connected))throw new AgentError('Orbit의 연결 → Google Calendar에서 연결해 주세요.');if(args.eventId){output=await inspectCalendarSeries(db,owner,env,args.eventId);}else{await syncCalendar(db,owner,env,job.planning?.date);output={events:(await readWorkspace(db,owner)).data.events.filter(e=>e.id.startsWith('google:'))};}pendingSources.push(sourceRecord('event',`google-read:${job.round}`,'Google 기본 캘린더',JSON.stringify(output).slice(0,1200),{label:'Google Calendar 조회 발췌'}));
