  ...
  if(job.phase==='read'){
   ...
   try{
    if(request.tool==='agent_orders'){
     ...
     z.object({}).strict().parse(request.arguments);const status=await connections(db,owner,env);if(!status.some(c=>c.provider==='google_calendar'&&c.connected))throw new AgentError('Google Calendar가 연결되어 있지 않습니다.');await syncCalendar(db,owner,env,job.planning?.date);output={events:(await readWorkspace(db,owner)).data.events.filter(e=>e.id.startsWith('google:'))};pendingSources.push(sourceRecord('event',`google-read:${job.round}`,'Google 기본 캘린더',JSON.stringify(output).slice(0,1200),{label:'Google Calendar 조회 발췌'}));
