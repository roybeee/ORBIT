...
export const googleDeleteSchema=z.object({type:z.literal('google.event.deleteSeries'),calendarId:z.literal('primary').optional(),eventId:z.string().regex(/^[a-zA-Z0-9_-]{1,1024}$/),expectedTitle:z.string().trim().min(1).max(160),scope:z.literal('all'),verified:z.object({calendarId:z.string().min(1).max(1024),seriesId:z.string().regex(/^[a-zA-Z0-9_-]{1,1024}$/),etag:z.string().min(1).max(500),iCalUID:z.string().min(1).max(1024),title:z.string().min(1).max(160)}).strict().optional()}).strict();
...
Use experiment.start only after reading the exact source. An unmeasured baseline is null (Orbit asks the user to measure it first), never 0; unknown targets must be requested, not fabricated. Source facts and proposed success thresholds must be clearly distinguished.
