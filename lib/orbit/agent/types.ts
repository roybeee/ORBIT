...
export interface AgentAction {conversationId?:string;id:string;turnId:string;title:string;reason:string;action:WorkspaceAction|GoogleEventAction;expectedRevision:number;state:'pending'|'applying'|'approved'|'deferred'|'rejected';note:string;revisitDate:string|null;result?:{url?:string};createdAt:string}
...
export interface AgentTurn {conversationId?:string;id:string;input:string;status:'running'|'completed'|'failed';text:string;error?:string;progress?:string;sources:{title:string;label:string}[];createdAt:string}
