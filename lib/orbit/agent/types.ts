...
export interface GoogleEventAction {type:'google.event.create';event:{title:string;date:string;start:number;end:number;timeZone:string;description:string;projectId?:string|null}}
...
export interface AgentState {activeRuns?:ActiveRun[];conversation?:Conversation|null;pendingActions?:AgentAction[];activeRun?:{id:string;conversationId:string}|null;turns:AgentTurn[];actions:AgentAction[];connections:Connection[];hasMore:boolean;nextBefore:string|null}
