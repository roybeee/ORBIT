...
export async function finishTurn(db:Database,owner:string,id:string,lease:string,response:Pick<AgentTurn,'text'|'sources'>,actions:AgentAction[],refreshActionId?:string){
 ...
 for(const action of actions){if(!action.guard&&snapshot?.revision===action.expectedRevision&&projected){action.guard=await guardFor(action.action,projected);try{if(!action.action.type.startsWith('google.')&&action.action.type!=='agent.dispatch')projected=applyAction(projected,action.action as Parameters<typeof applyAction>[1])}catch{}}}
