...
export interface GoogleEvent {orbitCalendarId?:string;id:string;summary?:string;status?:string;transparency?:string;attendees?:{self?:boolean;responseStatus?:string}[];start:{date?:string;dateTime?:string};end:{date?:string;dateTime?:string};htmlLink?:string;extendedProperties?:{private?:Record<string,string>}}
...
export function normalizeEvents(items:GoogleEvent[],timeZone:string,from:string,to:string):CalendarEvent[]{
 ...
 for(const event of items){if(!event.id||event.status==='cancelled'||event.transparency==='transparent'||event.attendees?.some(a=>a.self&&a.responseStatus==='declined'))continue;
  ...
  for(let date=first<from?from:first;date<=last&&date<to;date=addDays(date,1)){const a=date===first?start:0,b=date===last?end:1440;if(b>a)out.push({id:`google:${event.orbitCalendarId&&event.orbitCalendarId!=='primary'?encodeURIComponent(event.orbitCalendarId)+':':''}${event.id}:${date}`,google:{calendarId:event.orbitCalendarId??'primary',eventId:event.id,orbitEventId:event.extendedProperties?.private?.orbitEventId},title:(event.summary??'비공개 일정').slice(0,160),date,start:a,end:b,kind:'meeting',...(event.start.date&&event.end.date?{allDay:true}:{})});}
 }
