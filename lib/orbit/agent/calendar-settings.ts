...
export async function setCalendarSelection(db:Database,owner:string,env:Runtime,ids:string[]){
 ...
 const snapshot=await readWorkspace(db,owner),version=new Date().toISOString(),mutation=crypto.randomUUID();
 const result=await db.batch([
 db.prepare('INSERT INTO orbit_workspaces(owner_id,revision,state_json,mutation_id,updated_at) VALUES(?,?,?,?,?) ON CONFLICT(owner_id) DO UPDATE SET revision=excluded.revision,mutation_id=excluded.mutation_id,updated_at=excluded.updated_at WHERE orbit_workspaces.revision=?').bind(owner,snapshot.revision+1,JSON.stringify({...snapshot.data,events:snapshot.data.events.filter(e=>!e.id.startsWith('google:'))}),mutation,version,snapshot.revision),
