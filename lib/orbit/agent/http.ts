...
export async function owner(request?:Request){const user=await getChatGPTUser();if(!user)throw new AgentError('로그인이 필요합니다.','AUTH',401);const expected=request?.headers.get('x-orbit-owner')??(await headers()).get('x-orbit-owner');if(expected&&expected!==user.id)throw new AgentError('로그인 계정이 바뀌었습니다. 입력은 이전 계정의 임시 보관함에 남아 있습니다. 새로 열어 주세요.','SESSION_CHANGED',409);if(request){const origin=request.headers.get('origin');if((origin&&origin!==new URL(request.url).origin)||request.headers.get('sec-fetch-site')==='cross-site')throw new AgentError('요청 출처를 확인할 수 없습니다.','ORIGIN',403)}return user}
