 ...
 try{
  ...
  if(session){
   ...
   if(old?.signature===signature){state.pending.shift();}
    ...
    if(page.object!=='list'||!Array.isArray(page.data)||typeof page.session_id!=='string')throw new AgentError('Hermes 대화 원문 형식을 확인하지 못했습니다.','HERMES_FORMAT',502);
    ...
    if(page.session_id!==session.id){
     ...
     if(page.session_id!==session.id||!Array.isArray(page.data))throw new AgentError('압축된 대화가 변경되어 다음 수집에서 다시 확인합니다.','HERMES_FORMAT',502);
    ...
    const messages=page.data.map((m:Record<string,unknown>)=>{
     if(!['string','number'].includes(typeof m.id)||!['user','assistant','tool','system','function'].includes(String(m.role)))throw new AgentError('메시지 식별자를 확인하지 못했습니다.','HERMES_FORMAT',502);
     // Reasoning and system prompts never enter the record collection.
     const content=m.role==='system'?'[시스템 메시지 제외]':redactActivity(m.content,config.token);
     const tools=m.tool_calls?redactActivity(m.tool_calls,config.token):'';
     return {id:String(m.id),role:String(m.role),content:content.slice(0,30000)+(content.length>30000?'\n[긴 메시지 일부만 보관 · Hermes 원본 확인]':''),tool:String(m.tool_name??'').slice(0,150),tools:tools.slice(0,8000),at:String(m.timestamp??'')};
    });
