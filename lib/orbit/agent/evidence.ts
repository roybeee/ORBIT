import {operatingSignals} from '../phase3.ts';
import {todayInZone} from '../dates.ts';
import type { WorkspaceData, Note, Task } from '../model.ts';
import { AgentError } from './errors.ts';
export interface AgentSource {
  title:string; label:string;
  id?:string; kind?:'metric'|'note'|'task'|'project'|'goal'|'event'|'review'|'conversation'|'decision'|'delegation'|'plaud'|'memory';
...
export function catalogEvidence(data:WorkspaceData,registry:EvidenceRegistry) {
  return addEvidence(registry,[
    ...operatingSignals(data,todayInZone(data.preferences.timeZone)).map(s=>sourceRecord('metric',s.metric.id,s.metric.name,JSON.stringify(s),{date:s.latest?.through})),
