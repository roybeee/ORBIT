...
export const researchReply=z.discriminatedUnion('kind',[
 z.object({kind:z.literal('orbit.read'),notes:z.string().max(40000).optional(),requests:z.array(researchReadSchema).min(1).max(32)}).strict(),
...
export interface ResearchState {round:number;phase:'model'|'read';notes:string;queue:ResearchRead[];results:unknown[];invalid:number;formatStopped?:boolean;repairBaseInput?:string;formatIssue?:string}
...
Return 1..32 requests per orbit.read; Orbit runs them sequentially. Always include your updated notes when possible; omitting notes retains the previous notes. Do not invent wrapper fields or put an array/string in place of the JSON object. For example {"kind":"orbit.read","notes":"f2 needs transcript next page","requests":[{"tool":"plaud_read","arguments":{"name":"get_note","arguments":{"file_id":"actual-ID"}}},{"tool":"plaud_read","arguments":{"name":"get_transcript","arguments":{"file_id":"actual-ID","cursor":"exact provider cursor"}}}]} — the INNER provider arguments must match the discovered Plaud schema (file_id/cursor are examples only).
