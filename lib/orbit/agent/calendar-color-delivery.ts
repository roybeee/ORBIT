...
export async function deliverGoogleColor(db:Database,owner:string,env:Runtime,original:string){
 ...
 try{
  ...
  const calendar=await fetchJson('https://www.googleapis.com/calendar/v3/users/me/calendarList/'+encodeURIComponent(target.calendarId),{headers},6000);
  ...
  const current=await fetchJson(url,{headers},6000);
  ...
  if(String(saved.colorId??'')!==target.colorId){
   ...
   const changed=await fetchJson(url+'?sendUpdates=none',{method:'PATCH',headers:{...headers,'Content-Type':'application/json','If-Match':current.data.etag},body:JSON.stringify({colorId:target.colorId})},6000);
