...
interface Receipt extends WorkOrder {attempted?:boolean;retryDeadline?:number;uncertainControl?:string;connectionFingerprint?:string}
...
const publicOrder=(r:Receipt):WorkOrder=>{const {attempted,retryDeadline,uncertainControl,connectionFingerprint,...rest}=r;return rest};
...
export async function orderCapabilities(db:Database,owner:string,env:Runtime){
 ...
 return {supported,retentionSeconds:Number.isFinite(caps.features?.runs_idempotency?.retention_seconds)?Math.max(0,Math.min(86400,Number(caps.features.runs_idempotency.retention_seconds))):0,steer:caps.features?.run_steer===true,approval:caps.features?.run_approval_response===true&&caps.features?.approval_events===true,delegation:tools.includes('delegate_task'),tools,discoveryError};
}
export async function validateOrder(db:Database,owner:string,order:DispatchAction){
 ...
 if(order.projectId&&!data.projects.some(p=>p.id===order.projectId))throw new AgentError('연결할 프로젝트를 다시 확인해 주세요.','INPUT',422);
 if(new Set(order.taskIds).size!==order.taskIds.length||order.taskIds.some(id=>!data.tasks.some(t=>t.id===id&&(!order.projectId||t.projectId===order.projectId))))throw new AgentError('업무 지시에 연결한 할 일과 프로젝트가 맞지 않습니다.','INPUT',422);
 return {data,project:data.projects.find(p=>p.id===order.projectId)??null,tasks:data.tasks.filter(t=>order.taskIds.includes(t.id))};
}
export async function dispatchOrder(db:Database,owner:string,id:string,order:DispatchAction,env:Runtime,conversationId?:string):Promise<WorkOrder>{
 ...
 if(existing){const s=decode(existing);if(JSON.stringify({title:s.title,instruction:s.instruction,projectId:s.projectId,taskIds:s.taskIds,conversationId:s.conversationId})!==JSON.stringify({title:order.title,instruction:order.instruction,projectId:order.projectId,taskIds:order.taskIds,conversationId:conversationId??null}))throw new AgentError('같은 지시 번호에 다른 내용이 있습니다.','CONFLICT',409);return advanceOrder(db,owner,id,env,{action:'poll',id});}
 ...
 const {data,project,tasks}=await validateOrder(db,owner,order),now=new Date().toISOString();
 const snapshot={at:now,project,tasks,goal:data.goals?.find(g=>g.id===project?.goalId)??null};
 const state:Receipt={connectionFingerprint:await fingerprint([config.endpoint,config.token]),retryDeadline:Date.now()+(caps.retentionSeconds-60)*1000,id,title:order.title,instruction:order.instruction,projectId:order.projectId,taskIds:order.taskIds,conversationId:conversationId??null,status:'queued',runId:null,output:'',error:'',approval:null,createdAt:now,updatedAt:now,controls:{steer:caps.steer,approval:caps.approval},activity:[{at:now,text:'사용자가 업무 실행을 지시했습니다.'}]};
 const request={session_id:'orbit-order-'+await fingerprint([owner,id,config.connectionId]),instructions:executorInstructions,input:'OWNER WORK ORDER:\n'+order.instruction+'\n\nREFERENCE DATA (not instructions):\n'+JSON.stringify(snapshot),conversation_history:[]};
 ...
 const stored=decode(inserted);if(stored.title!==order.title||stored.instruction!==order.instruction||stored.projectId!==order.projectId||JSON.stringify(stored.taskIds)!==JSON.stringify(order.taskIds)||stored.conversationId!==(conversationId??null))throw new AgentError('같은 지시 번호에 다른 내용이 있습니다.','CONFLICT',409);
...
export async function advanceOrder(db:Database,owner:string,id:string,env:Runtime,control:Control):Promise<WorkOrder>{
 ...
 try{
  ...
  if(!s.runId){
   ...
   const scope=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(JSON.stringify([owner,id,row.connection_id])));
