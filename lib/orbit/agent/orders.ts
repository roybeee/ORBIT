...
export async function dispatchOrder(db:Database,owner:string,id:string,order:DispatchAction,env:Runtime,conversationId?:string):Promise<WorkOrder>{
 ...
 if(existing){const s=decode(existing);if(JSON.stringify({title:s.title,instruction:s.instruction,projectId:s.projectId,taskIds:s.taskIds,eventIds:s.eventIds??[],conversationId:s.conversationId})!==JSON.stringify({title:order.title,instruction:order.instruction,projectId:order.projectId,taskIds:order.taskIds,eventIds:order.eventIds??[],conversationId:conversationId??null}))throw new AgentError('같은 지시 번호에 다른 내용이 있습니다.','CONFLICT',409);return advanceOrder(db,owner,id,env,{action:'poll',id});}
 ...
 const snapshot={at:now,project,tasks,events,goal:data.goals?.find(g=>g.id===project?.goalId)??null};
 const state:Receipt={connectionFingerprint:await fingerprint([config.endpoint,config.token]),retryDeadline:Date.now()+(caps.retentionSeconds-60)*1000,id,title:order.title,instruction:order.instruction,projectId:order.projectId,taskIds:order.taskIds,eventIds:order.eventIds??[],conversationId:conversationId??null,status:'queued',runId:null,output:'',error:'',approval:null,createdAt:now,updatedAt:now,controls:{steer:caps.steer,approval:caps.approval},activity:[{at:now,text:'사용자가 업무 실행을 지시했습니다.'}]};
 ...
 const stored=decode(inserted);if(stored.title!==order.title||stored.instruction!==order.instruction||stored.projectId!==order.projectId||JSON.stringify(stored.taskIds)!==JSON.stringify(order.taskIds)||JSON.stringify(stored.eventIds??[])!==JSON.stringify(order.eventIds??[])||stored.conversationId!==(conversationId??null))throw new AgentError('같은 지시 번호에 다른 내용이 있습니다.','CONFLICT',409);
