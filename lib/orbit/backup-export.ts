...
const tables=['orbit_workspaces','orbit_note_revisions','orbit_reviews','orbit_conversations','orbit_agent_turns','orbit_agent_actions','orbit_agent_orders','orbit_order_reviews','orbit_order_reads','orbit_attachments','orbit_sound_state','orbit_calendar_settings','orbit_daily_runs','orbit_aside_jobs','orbit_source_status','orbit_calendar_exports','orbit_daily_runtime','orbit_chief_jobs','orbit_calendar_cache'] as const;
export async function backupArchive(db:Database,owner:string,bucket:Bucket){
 ...
 const stored=rows.orbit_workspaces[0],data=stored?JSON.parse(stored.state_json):emptyWorkspace();
