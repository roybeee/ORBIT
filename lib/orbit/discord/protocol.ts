...
export type Command={kind:'ask'|'run';text:string}|{kind:'help'|'status'}|{kind:'approve'|'reject'|'stop';id:string}|{kind:'defer';id:string;date:string;reason:string}|{kind:'allow'|'deny';id:string;requestId:string};
export function parseCommand(content:string):Command|null{
 ...
 const aliases:Record<string,string>={'질문':'ask','실행':'run','상태':'status','도움말':'help','승인':'approve','거절':'reject','보류':'defer','중지':'stop','허용':'allow','차단':'deny'};
 const kind=aliases[verb]??verb;
 if(!kind||kind==='help')return {kind:'help'};
 if(kind==='status')return {kind};
 if(kind==='ask'||kind==='run'){if(!rest||rest.length>8000)throw Error('질문 또는 업무 내용을 8,000자 이내로 입력하세요.');return {kind,text:rest};}
