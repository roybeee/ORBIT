...
export function executiveContext(data:WorkspaceData,date:string,full=false){
 const all=operatingSignals(data,date),operating=all.sort((a,b)=>Number(b.state==='attention')-Number(a.state==='attention')).slice(0,full?undefined:12).map(s=>({metricId:s.metric.id,name:s.metric.name,projectId:s.metric.projectId,unit:s.metric.unit,state:s.state,latest:s.latest?{value:s.latest.value,from:s.latest.from,through:s.latest.through,source:full?s.latest.source:s.latest.source.slice(0,300)}:null,baseline:s.baseline?{value:s.baseline.value,from:s.baseline.from,through:s.baseline.through}:null,delta:s.delta,percent:s.percent,question:s.question,hypothesis:s.hypothesis,followup:s.followup?{id:s.followup.id,status:s.followup.status,taskId:s.followup.taskId,delegationId:s.followup.delegationId}:null}));
 const plan=activeAllocation(data,date);
 return {weeklyAllocation:plan?{...plan,allocations:plan.allocations.map(a=>({...a,reason:full?a.reason:a.reason.slice(0,160)}))}:null,operating,operatingOmitted:Math.max(0,all.length-operating.length),meetingResults:(data.meetingRecords??[]).slice(full?0:-6).map(r=>({id:r.id,projectId:r.projectId,event:r.event,noteId:r.noteId,noteRevision:r.noteRevision,summary:full?r.summary:r.summary.slice(0,350),changedConditions:full?r.changedConditions:r.changedConditions.slice(0,350),decisionId:r.decisionId,taskIds:r.taskIds,delegationIds:r.delegationIds}))};
}
