...
export function applyAction(current:WorkspaceData,action:WorkspaceAction,now=new Date()):WorkspaceData{
 ...
 case 'review.save':
 case 'review.saveGenerate':{if(action.review.date>today)fail('미래 날짜의 회고는 아직 기록할 수 없습니다.');data.reviews=replace(data.reviews,{...action.review,id:action.review.date,completedIds:data.tasks.filter(t=>t.completedOn===action.review.date&&t.status==='done').map(t=>t.id),updatedAt:now.toISOString()});if(action.type==='review.save')break;const date=addDays(action.review.date,1);saveProposal(generateProposal(data.tasks,data.events,date,action.review.energy,data.proposals.find(p=>p.date===date),data.preferences));break;}
 case 'proposal.brief':saveProposal(planFromBrief(data,action.brief,action.energy));break;
