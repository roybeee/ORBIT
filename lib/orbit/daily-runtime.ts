...
// A run stopped by a provider limit waits for recovery without spending an attempt; the rule-based plan
// covers the day meanwhile and the full analysis resumes after the limit is back.
if(turn.status==='failed'&&r.date>=today&&waitsForLimit(String(response.error??''))){state.status='waiting_quota';state.attempts=Math.max(0,state.attempts-1);state.holdId=(await activeHold(db,owner,'hermes'))?.id??state.holdId;}
else state.status=turn.status==='failed'&&state.attempts<3&&!/중지|cancel/i.test(response.error??'')&&r.date>=today&&!snapshot.data.proposals.some(p=>p.date===r.date)?'queued':turn.status;await db.prepare('UPDATE orbit_daily_runs SET state_json=?,updated_at=? WHERE owner_id=? AND date=?').bind(JSON.stringify(state),new Date().toISOString(),owner,r.date).run();}}
 ...
 if(row){const state=JSON.parse(row.state_json),hold=state.status==='waiting_quota'?await activeHold(db,owner,'hermes'):null,resume=state.status==='waiting_quota'&&(!hold||Date.now()>=hold.nextCheckAt&&hold.probeUntil<=Date.now());if((state.status==='queued'||resume)&&state.attempts<3){state.attempts++;try{const planning={date:target,energy:snapshot.data.reviews.find(r=>r.date===addDays(target,-1))?.energy??'normal' as const};try{await runAgent(db,owner,{id:state.id,message:briefMessage(planning),planning,retryFailed:resume},env);state.status='running';}catch(e){if(e instanceof AgentError&&e.code==='HERMES_SETUP'){await localPlanning(db,owner,state.id+':local',planning);state.status='local';}else throw e;}}catch{state.message='자동 제안 준비 실패 · 연결을 확인한 뒤 내일 제안에서 다시 요청하세요.';if(state.attempts>=3)state.status='failed';}await db.prepare('UPDATE orbit_daily_runs SET state_json=?,updated_at=? WHERE owner_id=? AND date=?').bind(JSON.stringify(state),new Date().toISOString(),owner,target).run();return await finish(true);}}
