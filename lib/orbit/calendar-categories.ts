...
export function taskCalendarEvent(task:Task,today=task.due):CalendarEvent{return {id:'task-due:'+task.id,taskId:task.id,projectId:task.projectId,title:(task.status==='done'?'✓ ':'')+task.title,date:taskCalendarDate(task,today),start:0,end:1440,kind:'focus',allDay:true,color:task.color,category:categoryOf(task)}}
...
export function taskCalendarSource(task:Task,preferences:Preferences,today=todayInZone(preferences.timeZone)){const event=taskCalendarEvent(task,today);return JSON.stringify([event.title,event.date,event.category,calendarItemColor(task,preferences,'task')])}
