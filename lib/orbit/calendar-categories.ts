...
export function taskCalendarSource(task:Task,preferences:Preferences,today=todayInZone(preferences.timeZone)){const event=taskCalendarEvent(task,today);return JSON.stringify([event.title,event.date,event.category,calendarItemColor(task,preferences,'task'),event.description,event.scope])}
