...
export function calendarTimeline(tasks:Task[],events:CalendarEvent[],date:string,today:string):CalendarEvent[]{
 ...
 const due=tasks.filter(t=>taskCalendarDate(t,today)===date),dueIds=new Set(due.map(t=>t.id));
 const scheduled=events.filter(e=>e.date===date).flatMap(event=>{
  ...
  if(mirrorId&&dueIds.has(mirrorId)&&event.allDay)return [];
 ...
 return [...untimed,...scheduled.sort((a,b)=>Number(!!b.allDay)-Number(!!a.allDay)||a.start-b.start||a.end-b.end||a.id.localeCompare(b.id))];
}
