...
export interface BriefEvidence {id:string;kind:'metric'|'project'|'task'|'note'|'review'|'event'|'conversation'|'decision'|'delegation'|'plaud'|'memory'|'goal'|'rule';recordId:string;title:string;date?:string;revision?:number;excerpt?:string}
