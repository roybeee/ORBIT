...
export async function publishBrief(db:Database,owner:string,id:string,lease:string,brief:DailyBrief,planning:PlanningRequest){
 ...
 const now=new Date().toISOString(),revision=current.revision+1,storage=prepareWorkspace(next);
 ...
 try{
  const result=await db.batch([
   db.prepare(`INSERT INTO orbit_workspaces(owner_id,revision,state_json,mutation_id,updated_at)
    ...
    ON CONFLICT(owner_id) DO UPDATE SET revision=excluded.revision,state_json=excluded.state_json,mutation_id=excluded.mutation_id,updated_at=excluded.updated_at WHERE orbit_workspaces.revision=?`).bind(owner,revision,storage.stateJson,id,now,owner,id,lease,current.revision),
