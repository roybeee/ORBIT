...
export async function collectPlanningContext(db:Database,owner:string,snapshot:WorkspaceSnapshot,request:PlanningRequest,connected:Connection[],env:Runtime,warnings:string[]=[],budget:PlanningBudget=PLANNING_BUDGETS[0]):Promise<PlanningContext>{
 ...
 const brainy={dominoProject:domino?{id:domino.id,name:domino.name,goal:domino.goal}:null,goals:(data.goals??[]),laserMinutes:prefs.laserMinutes,rhythm:prefs.rhythm,rules:(data.improvements??[]).filter(i=>i.active).map(i=>({id:i.id,rule:i.rule,kind:i.kind,...(i.experimentId?{experimentId:i.experimentId}:{}),...(i.effect?{appliedByOrbit:i.effect}:{}),evidence:add('rule',i.id,i.rule,{date:i.createdOn,excerpt:i.experimentId?'실험 결과를 보고 채택한 규칙':'사용자가 정한 규칙'})})),habits:(data.habits??[]).map(h=>({title:h.title,mode:h.mode,streak:habitStreak(h,cutoff)})),risks:(data.risks??[]),week:weeklyStats(data,cutoff),yesterday:data.reviews.filter(r=>r.date<=cutoff).slice(-1).map(r=>({date:r.date,stats:r.stats??null,highlight:r.highlight??null}))[0]??null};
 ...
 const transmitted=new Set([...operating,...decisions,...delegations,...projects,...tasks,...documents,...reviews,...events,...conversations,...memoryRefs,...goalRefs,...brainy.rules].map(record=>record.evidence));
