...
export function planFromBrief(data:WorkspaceData,brief:DailyBrief,energy:Proposal['energy'],now=new Date()):Proposal{
 ...
 if(existing&&!workEligibility(data,existing,brief.date).allowed)throw new DomainError('완료·보류·대기·선행 작업을 반영해 제안을 다시 분석해 주세요.');
 ...
 const proposal=generateProposal(pool,[...data.events,...careEvents(data,brief.date),...protectedEvents(data,brief.date)],brief.date,energy,previous,data.preferences,{context:data,earliestStart:brief.date<todayInZone(data.preferences.timeZone,now)?0:planningFloor(brief.date,data.preferences.timeZone,now),ordered,laserTaskId,dominoProjectId:data.dominoProjectId,calibration:t=>calibrationFactor(data.tasks,t,brief.date,data.executionHistory),rules:data.improvements});
