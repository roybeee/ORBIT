import {planningFloor,todayInZone} from '../dates.ts';
import {allocationAllowsWork,protectedEvents} from '../phase3.ts';
...
export function planFromBrief(data:WorkspaceData,brief:DailyBrief,energy:Proposal['energy'],now=new Date()):Proposal{
 ...
 for(const priority of brief.priorities){
  ...
  if(!goalAllowsWork(data,project.id)||!allocationAllowsWork(data,project.id,brief.date))throw new DomainError('보류하거나 달성한 목표의 작업입니다. 제안을 다시 분석해 주세요.');
  ...
  if(existing&&(!!existing.blocker?.trim()||existing.status==='done'||existing.status==='waiting'||(existing.planHoldUntil&&existing.planHoldUntil>brief.date)||(existing.dependsOn??[]).some(id=>data.tasks.find(t=>t.id===id)?.status!=='done')))throw new DomainError('완료·보류·대기·선행 작업을 반영해 제안을 다시 분석해 주세요.');
 ...
 const pool=[...data.tasks.filter(t=>t.status==='done'||goalAllowsWork(data,t.projectId)&&allocationAllowsWork(data,t.projectId,brief.date)),...drafts];
 ...
 const proposal=generateProposal(pool,[...data.events,...careEvents(data,brief.date),...protectedEvents(data,brief.date)],brief.date,energy,previous,data.preferences,{earliestStart:brief.date<todayInZone(data.preferences.timeZone,now)?0:planningFloor(brief.date,data.preferences.timeZone,now),ordered,laserTaskId,dominoProjectId:data.dominoProjectId,calibration:t=>calibrationFactor(pool,t,brief.date)});
