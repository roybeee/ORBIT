...
export function planFromBrief(data:WorkspaceData,brief:DailyBrief,energy:Proposal['energy']):Proposal{
 ...
 const proposal=generateProposal(pool,[...data.events,...careEvents(data,brief.date),...protectedEvents(data,brief.date)],brief.date,energy,previous,data.preferences,{ordered,laserTaskId,dominoProjectId:data.dominoProjectId,calibration:t=>calibrationFactor(pool,t,brief.date)});
