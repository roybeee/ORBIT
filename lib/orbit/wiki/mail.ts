import {mailMetadata} from '../replies.ts';
import {readWorkspace,writeCommand,readNote,type Database} from '../../../db/repository.ts';
...
export async function syncWikiMail(db:Database,owner:string,env:Runtime) {
 ...
 for(const item of listed.data.messages??[]) {
  ...
  await writeCommand(db,owner,{operationId:'wiki-mail:'+item.id,expectedRevision:snapshot.revision,action:{type:'note.upsert',note:{id,title,kind:'meeting',projectId,summary:String(m.snippet??'').slice(0,500),body:`발신: ${header('from')}\n수신: ${header('to')}\n날짜: ${header('date')}\n\n${text||m.snippet||'본문 없음'}${text?'':'\n\n[텍스트 본문이 없어 메일 요약만 수집했습니다.]'}`,tags:['메일','Gmail'],updated:day,source:{...(mail?{mail}:{}),provider:'gmail',externalId:item.id,date:day,url:'https://mail.google.com/mail/u/0/#all/'+item.id}}}});count++;
 }
 await db.prepare("UPDATE orbit_integrations SET public_json=json_set(public_json,'$.wikiSyncAt',?,'$.wikiPage',?,'$.wikiCycle',?) WHERE owner_id=? AND provider='google_mail'").bind(Date.now(),!catchup&&state.wikiPage?state.wikiPage:listed.data.nextPageToken??null,Number(state.wikiCycle??0)+1,owner).run();
