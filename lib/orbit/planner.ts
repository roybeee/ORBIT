import type {Task,CalendarEvent,ProposalItem,Proposal} from './model';
export function overlaps(a:{start:number;end:number},b:{start:number;end:number}){return a.start<b.end&&a.end>b.start;}
export function availableWindows(events:CalendarEvent[],date:string){
  const busy=events.filter(e=>e.date===date&&e.end>540&&e.start<1080).map(e=>({start:Math.max(540,e.start),end:Math.min(1080,e.end)})).sort((a,b)=>a.start-b.start);
  let cursor=540;const windows:{start:number;end:number}[]=[];
  for(const event of busy){if(event.start>cursor)windows.push({start:cursor,end:event.start});cursor=Math.max(cursor,event.end);}
  if(cursor<1080)windows.push({start:cursor,end:1080});return windows;
}
export function generateProposal(tasks:Task[],events:CalendarEvent[],date:string,energy:Proposal['energy']='normal',previous?:Proposal):Proposal{
  ...
  const windows=availableWindows(reservations,date);
  const free=windows.reduce((sum,w)=>sum+w.end-w.start,0);
  const budget=Math.floor(free*({low:.5,normal:.8,high:.85}[energy]));
  let remaining=budget;
  const isReady=(t:Task)=>t.status!=='waiting'&&t.status!=='done'&&!(t.dependsOn??[]).some(id=>tasks.find(t=>t.id===id)?.status!=='done');
  const candidates=tasks.filter(t=>isReady(t)&&!retained.some(i=>i.taskId===t.id)&&!events.some(e=>e.date===date&&e.taskId===t.id));
  const score=(t:Task)=>t.impact*10+(t.due<=date?30:0)+(t.focus?15:0)+(t.status==='doing'?5:0);
  candidates.sort((a,b)=>score(b)-score(a)||a.due.localeCompare(b.due)||a.id.localeCompare(b.id));
  const items=[...retained];const unscheduled:string[]=[];
  for(const task of candidates){
    const slot=windows.find(w=>w.end-w.start>=task.duration);
    if(!Number.isFinite(task.duration)||task.duration<1||task.duration>remaining||!slot||items.filter(i=>i.state!=='deferred').length>=3){unscheduled.push(task.id);continue;}
    ...
    slot.start=end+10;remaining-=task.duration;
  }
  return {id:`plan:${date}`,date,items:items.sort((a,b)=>a.start-b.start),unscheduled,budget,energy};
}
export function approveProposalItem(proposal:Proposal,itemId:string,tasks:Task[],events:CalendarEvent[]):{proposal:Proposal;events:CalendarEvent[];error?:string}{
  ...
  if(task.status==='done'||task.status==='waiting'||(task.dependsOn??[]).some(id=>tasks.find(t=>t.id===id)?.status!=='done'))return {proposal,events,error:'업무 상태가 바뀌었습니다. 제안을 다시 생성해 주세요.'};
