...
export const metricSchema=z.object({id,projectId:id,name:text(100),category:z.enum(['sales','cost','evidence','people','contract']),unit:text(20),badDirection:z.enum(['up','down']),thresholdPercent:z.number().min(0).max(10000),thresholdAbsolute:z.number().finite().min(0).max(1e15),maxAgeDays:z.number().int().min(1).max(366),assignee:z.string().trim().max(100)}).strict();
