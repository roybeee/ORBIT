...
import {collectNotifications,listNotifications,readNotifications,dismissNotifications,notify} from '@/lib/orbit/notifications/store';
...
const input=z.discriminatedUnion('action',[
 ...
 z.object({action:z.literal('dismiss'),ids:z.array(z.string().max(500)).min(1).max(100)}).strict(),
...
export async function POST(request:Request){try{const user=await owner(request),parsed=input.safeParse(await body(request,12000));if(!parsed.success)throw new AgentError('알림 요청을 확인해 주세요.');const db=getDatabase(),v=parsed.data;
 ...
 if(v.action==='dismiss')await dismissNotifications(db,user.id,v.ids);
