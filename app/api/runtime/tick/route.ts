...
export async function POST(request:Request){
 ...
 try{return Response.json(await tickRuntime(getDatabase(),owner,env,{scheduled:true}),{headers:{'Cache-Control':'no-store'}})}catch{return Response.json({error:'Tick failed'},{status:503});}
}
