...
export async function POST(request:Request){
 ...
 try{const action=parsed.data.action;if(action.type==='proposal.generate'||action.type==='review.saveGenerate')return response((await startPlanningAction(getDatabase(),user.id,{...parsed.data,action},env)).snapshot);const db=getDatabase(),snapshot=await writeCommand(db,user.id,parsed.data);if(snapshot.data.tasks.length||['proposal.approve','event.upsert','task.delete'].includes(action.type))after(()=>flushCalendarOutbox(db,user.id,env,action.type==='task.schedule'?action.eventId:action.type==='event.upsert'?action.event.id:action.type==='proposal.approve'?'approved:'+action.itemId:action.type==='task.upsert'?'task-due:'+action.task.id:undefined).then(()=>{}));return response(snapshot)}
