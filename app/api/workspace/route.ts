...
export async function POST(request:Request){
 ...
 try{const action=parsed.data.action;if(action.type==='proposal.generate'||action.type==='review.saveGenerate')return response((await startPlanningAction(getDatabase(),user.id,{...parsed.data,action},env)).snapshot);const db=getDatabase(),snapshot=await writeCommand(db,user.id,parsed.data);if(['proposal.approve','event.upsert'].includes(action.type))after(()=>flushCalendarOutbox(db,user.id,env,action.type==='event.upsert'?action.event.id:action.type==='proposal.approve'?'approved:'+action.itemId:undefined).then(()=>{}));return response(snapshot)}
