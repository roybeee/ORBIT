...
export async function GET(request:Request){
 ...
 if(!id){const projectId=params.get('project')??'',tag=params.get('tag')??'',documentKind=params.get('documentKind')??'';if(projectId.length>100||tag.length>40||!['','wiki','meeting','knowledge'].includes(documentKind))return response({error:'검색 조건을 확인해 주세요.'},400);const query=params.get('q')??'',kind=params.get('kind')??'wiki',offset=Number(params.get('offset')??0),expected=params.get('workspaceRevision');if(query.length>200||!['wiki','knowledge','all'].includes(kind)||!Number.isSafeInteger(offset)||offset<0||offset>100000||(expected!==null&&(!/^\d+$/.test(expected)||!Number.isSafeInteger(Number(expected)))))return response({error:'검색 조건을 확인해 주세요.'},400);try{return response(await searchNotes(getDatabase(),user.id,{query,kind:kind as 'wiki'|'knowledge'|'all',offset,projectId,tag,documentKind,expectedRevision:expected===null?undefined:Number(expected)}))}catch(error){if(error instanceof RevisionConflict)return response({error:error.message},409);console.error('Orbit note search unavailable');return response({error:'검색을 완료하지 못했습니다. 다시 시도해 주세요.'},503)}}
