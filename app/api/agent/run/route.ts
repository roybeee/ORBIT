...
export async function POST(request:Request){try{const user=await owner(request),input=inputSchema.safeParse(await body(request,1000));if(!input.success)throw new AgentError('실행할 대화를 확인해 주세요.');if(input.data.action==='cancel')await advanceAgent(getDatabase(),user.id,input.data.id,env,true);else await driveAgent(getDatabase(),user.id,input.data.id,env);return json({ok:true})}catch(error){return failure(error)}}
