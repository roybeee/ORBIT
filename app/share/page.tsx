...
async function SharedWorkspace({draft}:{draft:string}){const user=await requireChatGPTUser('/share'+(draft?'?draft='+encodeURIComponent(draft):''));return <Workspace ownerId={user.id} displayName={user.fullName??user.email}/>}
