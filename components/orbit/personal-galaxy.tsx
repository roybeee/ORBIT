...
export function PersonalGalaxy({data,navigate,onOpen}:{data:WorkspaceData;navigate:(view:View)=>void;onOpen:(target:{kind:'project';id:string})=>void}) {
  ...
  const projects=[...data.projects].sort((a,b)=>b.priority-a.priority).slice(0,5);
  ...
  return <section className="personal-galaxy" aria-labelledby="galaxy-title">
    ...
    <div className={'galaxy-map '+(!projects.length?'is-empty':'')}>
      <svg className="galaxy-paths" viewBox="0 0 600 310" preserveAspectRatio="none" aria-hidden="true">
        <ellipse cx="300" cy="149" rx="124" ry="64"/><ellipse cx="300" cy="149" rx="233" ry="118"/>
        {projects.map((p,i)=><line key={p.id} className={focused===p.id?'is-focused':''} x1="300" y1="149" x2={positions[i][0]*6} y2={positions[i][1]*3.1}/>)}
      </svg>
      <button className="galaxy-self" onClick={()=>navigate('goals')} aria-label="나의 목표 열기"><Orbit size={29}/><strong>나</strong><span>모든 연결의 시작</span></button>
      ...
      {!projects.length&&<p className="galaxy-empty">첫 프로젝트를 추가하면<br/>나의 우주가 넓어집니다.</p>}
    </div>
    <div className="galaxy-footer"><button onClick={()=>navigate('projects')}><FolderKanban size={15}/><strong>{data.projects.length}</strong> 프로젝트</button><button onClick={()=>navigate('tasks')}><CheckCheck size={15}/><strong>{remaining}</strong> 남은 실행</button><button onClick={()=>navigate('wiki')}><BookOpen size={15}/><strong>{data.notes.length}</strong> 기록</button></div>
    {data.projects.length>5&&<button className="galaxy-more text-button" onClick={()=>navigate('projects')}>프로젝트 {data.projects.length}개 전체 보기 <ArrowUpRight size={14}/></button>}
  </section>;
}
