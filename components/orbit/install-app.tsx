...
// The app address is client-only: the server snapshot stays empty so hydration matches, then the client fills it in.
const noSubscribe=()=>()=>{};
const appUrl=()=>new URL('/',location.origin).href;
const noUrl=()=>'';
const rememberedDismissal=()=>{try{return sessionStorage.getItem('orbit-install-later')==='1'}catch{return false}};
...
export function InstallBanner(){
 ...
 // ready is false until the PWA provider mounts, so this storage read never affects server or hydration output.
 const [dismissed,setDismissed]=useState(rememberedDismissal);
...
export function InstallRootHint(){
 ...
 useEffect(()=>{
  // eslint-disable-next-line react-hooks/set-state-in-effect -- reads and strips the ?install/?browser parameters after mount; the URL is not available during server rendering and the state follows that one-time read
...
export default function InstallApp(){
 ...
 const url=useSyncExternalStore(noSubscribe,appUrl,noUrl);
 useEffect(()=>{
  if(!ready)return;
  ...
  // eslint-disable-next-line react-hooks/set-state-in-effect -- the preselected device comes from the ?device parameter and the detected platform, both known only after mount
 ...
 return <main className="install-page">
  <header className="install-header"><Link href="/" className="text-button"><ArrowLeft size={17}/>내 워크스페이스</Link><span>MAC · WINDOWS · MOBILE</span></header>
  ...
  <section className="install-card install-routine-card" aria-labelledby="start-heading"><div className="install-section-title"><span>02</span><h2 id="start-heading">설치 후에도 같은 Orbit</h2></div><div className="install-routine"><Link href="/#agent"><span>대화</span><div><strong>헤르메스와 다음 행동 정리</strong><p>연결 설정과 저장된 대화를 이어갑니다.</p></div><ArrowRight size={17}/></Link><Link href="/#review"><span>저녁</span><div><strong>오늘을 돌아보는 5분</strong><p>해낸 일, 막힌 일과 에너지를 기록합니다.</p></div><ArrowRight size={17}/></Link><Link href="/#proposal"><span>내일</span><div><strong>제안을 승인하거나 보류</strong><p>승인한 집중 시간이 일정에 반영됩니다.</p></div><ArrowRight size={17}/></Link></div></section>
  <ShareSetup/>
