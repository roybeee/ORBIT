...
import {groupNotifications} from '@/lib/orbit/notifications/grouping';
...
export interface NewsSummary {unread:number;items:OrbitNotification[]}
// With trigger={false} the bell is hidden and the list opens from the 결재함 (소식) or 나.
export function NotificationCenter({ownerId,demo,trigger=true,open:openProp,onOpenChange,onSummary}:{ownerId:string;demo:boolean;trigger?:boolean;open?:boolean;onOpenChange?:(open:boolean)=>void;onSummary?:(summary:NewsSummary)=>void}){
 const [innerOpen,setInnerOpen]=useState(false),open=openProp??innerOpen;
 // Stable across renders: the polling effect depends on it, and each run of that effect
 // re-reads the list (GET collects new notifications server-side).
 const controls=useRef({openProp,onOpenChange,onSummary}),summarized=useRef('');
 useLayoutEffect(()=>{controls.current={openProp,onOpenChange,onSummary}});
 const setOpen=useCallback((value:boolean)=>{controls.current.onOpenChange?.(value);if(controls.current.openProp===undefined)setInnerOpen(value)},[]);
 const [data,setData]=useState<Inbox|null>(null),[error,setError]=useState(''),[busy,setBusy]=useState(false),[push,setPush]=useState(false),[supported,setSupported]=useState(false),[filter,setFilter]=useState('all');
 ...
 if(demo)return null;
 return <>{trigger&&<button className="icon-button notification-bell" aria-label={`알림함${data?.unread?' · 읽지 않은 알림 '+data.unread+'건':''}`} onClick={()=>{setOpen(true);void refresh()}}><Bell size={20}/>{!!data?.unread&&<span>{data.unread>99?'99+':data.unread}</span>}</button>}
 <Dialog open={open} onOpenChange={setOpen}><DialogContent className="notification-dialog"><DialogHeader><DialogTitle>소식 {data?.unread?`· ${data.unread}`:''}</DialogTitle><DialogDescription>회의 요약, 승인 요청, 업무 완료와 실패를 확인하세요. 결정이 필요한 항목은 결재함에서 처리합니다.</DialogDescription></DialogHeader>
