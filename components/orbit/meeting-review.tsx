...
export function MeetingReview({noteId,revision,demo,onChanged}:{noteId:string;revision:number;demo:boolean;onChanged?:()=>void}){
 ...
 {item.state==='pending'&&<div className="sheet-actions"><button className="primary-button" disabled={!!busy||!!prerequisite||data.stale||item.guard?.meeting?.needsDue} onClick={()=>void decide(item,'approve')}><Check size={15}/>승인하고 등록</button><button className="secondary-button" disabled={!!busy} onClick={()=>setHeld(held===item.id?'':item.id)}>보류</button><button className="text-button" disabled={!!busy} onClick={()=>void decide(item,'reject')}>반려</button></div>}
