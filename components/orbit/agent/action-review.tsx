...
export function ActionPreview({item,snapshot}:{item:AgentAction;snapshot:WorkspaceSnapshot}){
 ...
 const allRows=action.type==='task.upsert'?entries(action.task as unknown as Record<string,unknown>):action.type==='task.assign'?action.assignments.map((a):[string,unknown]=>['할 일 연결',`${snapshot.data.tasks.find(t=>t.id===a.id)?.title??a.id} → ${projectName(a.projectId)}`]):entries(action as unknown as Record<string,unknown>);
 // A meeting card without a due date carries a placeholder due; show it as undecided, not as a date.
 const rows=item.guard?.meeting?.needsDue?allRows.map(([key,value]):[string,unknown]=>key==='due'?[key,'미정 · 승인 전 지정']:[key,value]):allRows;
...
export function ActionCard({item,snapshot,review,lead,extra,onAskOther}:{item:AgentAction;snapshot:WorkspaceSnapshot;review:ActionReview;lead?:ReactNode;extra?:ReactNode;onAskOther?:(item:AgentAction)=>void}){
 ...
 // A meeting proposal without a due date cannot be approved as is: the date is saved, then approved.
 const meeting=item.guard?.meeting,needsDue=item.state==='pending'&&!!meeting?.needsDue;
 const [due,setDue]=useState(''),[dueError,setDueError]=useState('');
 async function approveWithDue(){
  if(!meeting||!due)return;setDueError('');
  try{await agentRequest('/api/meetings/review','PATCH',{noteId:meeting.noteId,actionId:item.id,due});await decide(item,'approve')}
  catch(e){setDueError(e instanceof Error?e.message:'마감일을 저장하지 못했습니다.')}
 }
 return <article className={'agent-action '+item.state}><div className="agent-action-top"><span>{actionKind(item)}</span><small>{({pending:'승인 대기',applying:'적용 중',approved:'반영 완료',deferred:'보류',rejected:'닫힘'})[item.state]}</small></div>{lead}<h3>{item.title}</h3><p>{item.reason}</p><ActionPreview item={item} snapshot={snapshot}/>{notice(item,onAskOther)}{item.note&&<p className="agent-defer-note">{item.revisitDate} 다시 검토 · {item.note}</p>}{extra}{['pending','applying'].includes(item.state)&&<div className="agent-action-buttons">{needsDue?<form className="agent-due-approve" onSubmit={e=>{e.preventDefault();void approveWithDue()}}><input type="date" required aria-label={item.title+' 마감일'} value={due} onChange={e=>setDue(e.target.value)}/><button className="primary-button" disabled={!!acting||!due}>{acting===item.id?<LoaderCircle size={15} className="animate-spin"/>:<Check size={15}/>} 마감일 지정하고 승인</button></form>:<button className="primary-button" disabled={!!acting} onClick={()=>void decide(item,'approve')}>{acting===item.id?<LoaderCircle size={15} className="animate-spin"/>:<Check size={15}/>} {item.state==='applying'?'결과 확인 / 재시도':item.action.type==='agent.dispatch'?'승인하고 실행':'승인하고 반영'}</button>}{dueError&&<p role="alert" className="agent-error">{dueError}</p>}<button className="secondary-button" disabled={!!acting||item.state==='applying'} onClick={()=>openDefer(item)}><Clock3 size={15}/> 보류</button><button className="icon-button" aria-label={item.title+' 제안 닫기'} disabled={!!acting||item.state==='applying'} onClick={()=>void decide(item,'reject')}><X size={16}/></button></div>}{item.state==='deferred'&&<button className="text-button" disabled={!!acting} onClick={()=>void decide(item,'reconsider')}>다시 검토하기 <ArrowUpRight size={14}/></button>}</article>;
}
