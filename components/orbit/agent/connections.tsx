...
export function Connections({connections,onClose,onChange,calendarOnly=false}:{connections:Connection[];onClose:()=>void;onChange:()=>Promise<void>;calendarOnly?:boolean}){
 ...
 return <Dialog open onOpenChange={open=>{if(!open)onClose()}}><DialogContent className="agent-connection-dialog"><DialogHeader><DialogTitle>{calendarOnly?'Google 캘린더 연결':'Orbit 연결'}</DialogTitle><DialogDescription>{calendarOnly?'사용할 계정과 캘린더를 선택하세요.':'내 헤르메스와 기록을 연결하세요. 연결 암호와 계정 권한은 서버에 암호화해 보관합니다.'}</DialogDescription></DialogHeader><div className="dialog-form connection-list">
  {!calendarOnly&&<>
  ...
  <section className="connection-card"><header><span className="connection-icon"><CalendarDays size={22}/></span><div><h3>Google Calendar</h3><p>ORBIT에서 저장한 일정 자동 등록·수정</p></div><span className={'connection-status '+(state('google_calendar')?.connected?'connected':'')}>{state('google_calendar')?.connected?'연결됨':'연결 필요'}</span></header>
   <p className="connection-help">선택한 캘린더의 바쁨 일정을 함께 표시하고 시간 충돌을 확인합니다. Google 일정 생성 카드를 승인하면 내 캘린더에 등록합니다.</p>
