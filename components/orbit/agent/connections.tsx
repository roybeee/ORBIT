...
export function Connections({connections,onClose,onChange}:{connections:Connection[];onClose:()=>void;onChange:()=>Promise<void>}){
 ...
 async function connect(provider:'plaud'|'google_calendar'){const data=await agentRequest('/api/integrations/connect','POST',{provider});const url=new URL(data.url);if(!['https://mcp.plaud.ai','https://accounts.google.com'].includes(url.origin))throw new Error('연결 주소를 확인할 수 없습니다.');setAuthLink(url.href);setFeedback('인증 화면으로 이동합니다. 화면이 바뀌지 않으면 아래 링크를 누르세요.');window.location.assign(url.href)}
