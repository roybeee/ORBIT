...
export function useAgentConversations(demo:boolean,ownerId:string){
 ...
 // eslint-disable-next-line react-hooks/set-state-in-effect -- the initial filter comes from the URL and session storage, which are only readable after mount
 ...
 const runsRef=useRef(state.activeRuns??[]);
 useLayoutEffect(()=>{runsRef.current=state.activeRuns??[]},[state.activeRuns]);
