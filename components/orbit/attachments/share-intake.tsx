...
import {sharedDrafts,removeShare,claimShare,type ShareDraft} from '@/lib/orbit/share-drafts';
...
interface Props {
 snapshot:WorkspaceSnapshot;demo:boolean;ownerId:string;
...
export function ShareIntake(props:Props){
 ...
 // eslint-disable-next-line react-hooks/set-state-in-effect -- the intake opens only on the /share route, which is known from location after mount
 ...
 return <>
  ...
  <Dialog open={open} onOpenChange={value=>{if(!value&&!saving)close()}}><DialogContent className="share-intake-dialog">
   ...
   <p className="file-hint">공유 원본은 이 기기에 임시 보관됩니다. 직접 정리하거나 저장을 마칠 때까지 유지합니다. 대화 보내기나 일정 저장을 마치면 목록에서 정리됩니다.</p>
  </DialogContent></Dialog>
 </>;
}
function SharedItem({draft,ownerId,snapshot,onChat,onEvent,onDone,onBack,onDiscard,saving,setSaving}:Props&{draft:ShareDraft;saving:boolean;setSaving:(value:boolean)=>void;onDone:(committed:boolean)=>Promise<void>;onBack:()=>void;onDiscard:()=>Promise<void>}){
 ...
 useEffect(()=>{void agentRequest('/api/agent/conversations').then(r=>setConversations(r.items)).catch(()=>{})},[]);
 ...
 return <div className="share-review">
  ...
  {!claimed&&<div><p>{draft.files.map(f=>f.file.name).join(', ')}</p><button className="primary-button" disabled={saving} onClick={async()=>{setSaving(true);try{await claimShare(draft.id,ownerId);setClaimed(true);if(!started.current){started.current=true;upload.add(draft.files.map(f=>f.file),draft.files.map(f=>f.id));}}catch(e){setError(e instanceof Error?e.message:'보관 계정을 확인해 주세요.')}finally{setSaving(false)}}}>현재 로그인 계정으로 보관 시작</button></div>}
  {claimed&&<AttachmentInput scope={scope} disabled={saving}/>}
