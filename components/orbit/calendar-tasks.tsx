...
export function CalendarTasks({tasks,projects,preferences,date,today,disabled,onOpen,onToggle}:{tasks:Task[];projects:Project[];preferences:Preferences;date:string;today:string;disabled:boolean;onOpen:(id:string)=>void;onToggle:(id:string)=>void}){
 ...
 const row=(task:Task)=>{
  ...
  return <article key={task.id} className={`calendar-task-card ${done?'is-done':''}`} style={{'--task-category-color':categoryColor(category,preferences)} as CSSProperties}>
   ...
   <button className="calendar-task-open" disabled={disabled} onClick={()=>onOpen(task.id)}><strong>{task.title}</strong><span>{project?.name??'개인 할 일'} · {categoryLabels[category]} · {task.duration}분</span>
    {!done&&task.due<date?<span className="calendar-task-carried"><RotateCcw size={14}/>이월 · 원래 마감 {task.due.slice(5).replace('-','/')} · {statusLabel[task.status]}</span>:<span>{done?'완료':statusLabel[task.status]}</span>}
   </button><ChevronRight size={17} aria-hidden="true"/>
  </article>;
 };
 return <div className="calendar-task-list">
  <div className="section-title"><h2>{Number(date.slice(-2))}일 할 일</h2><span className="muted">미완료 {pending.length}개</span></div>
  <p className="calendar-task-guidance">{date===today&&carried.length?`이전 날짜의 미완료 ${carried.length}개가 이월됐어요. `:''}완료하지 않은 할 일은 다음 날로 자동 이월됩니다.</p>
  {pending.map(row)}
  {!pending.length&&<div className="calendar-empty"><CheckCheck size={28}/><strong>{completed.length?'이날 할 일을 모두 완료했어요':'이날 남은 할 일이 없어요'}</strong></div>}
  {!!completed.length&&<details className="calendar-tasks-done"><summary>완료한 할 일 {completed.length}개</summary>{completed.map(row)}</details>}
 </div>;
}
