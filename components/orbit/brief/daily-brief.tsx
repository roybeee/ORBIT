...
import {formatTime,durationText,type WorkspaceSnapshot,type View,type Proposal} from '@/lib/orbit/model';
import type {BriefRun,BriefEvidence} from '@/lib/orbit/brief/schema';
import type {WorkspaceAction} from '@/lib/orbit/validation';
import {addDays,koreanDate,todayInZone} from '@/lib/orbit/dates';
...
export function DailyBriefPanel({snapshot,date,setDate,energy,setEnergy,busy,demo,refresh,perform,navigate,open,launch}:Props){
 const [run,setRun]=useState<BriefRun|null>(null),[error,setError]=useState(''),[starting,setStarting]=useState(false),[defer,setDefer]=useState<string|null>(null),[reason,setReason]=useState(''),[revisit,setRevisit]=useState(addDays(date,1));
 ...
 const minDate=addDays(todayInZone(snapshot.data.preferences.timeZone),1);
 ...
 return <section className="daily-brief-surface">
  ...
  {(running||starting)&&<div className="brief-progress" role="status"><LoaderCircle size={20} className="animate-spin"/><div><strong>목표에 가까워지는 내일을 설계하고 있습니다</strong><p>{run?.progress??'회의록·일정·완료와 미완료 업무·회고를 모으고 있습니다.'}</p><small>이 화면을 다시 열어도 분석을 이어서 확인할 수 있습니다.</small></div>{run&&<button className="text-button" onClick={async()=>{try{await agentRequest('/api/agent/run','POST',{id:run.id,action:'cancel'})}catch(e){setError(e instanceof Error?e.message:'중지 상태 확인 실패')}}}>중지</button>}</div>}
  {brief?<article className="brief-paper">
   ...
   <section className="brief-progress-notes"><h3>오늘까지, 달라진 것</h3>{brief.progress.map((item,i)=><div key={i}><p>{item.text}</p>{evidence(item.evidence)}</div>)}</section>
   <section className="brief-priorities"><div className="brief-section-heading"><h3>내일의 우선순위와 실행 방안</h3><span>추가 집중 예산 {durationText(plan?.budget??0)}</span></div>
    {brief.priorities.map((priority,index)=>{
     ...
     return <section className="brief-priority" key={index}><div className="brief-priority-title"><b>{String(index+1).padStart(2,'0')}</b><div><span>{project?.name}</span><h4>{priority.title}</h4></div><span className="brief-time"><Clock3 size={14}/>{item?formatTime(item.start)+'–'+formatTime(item.end):'시간 배치 필요'}</span></div>
      <div className="brief-priority-body"><div><label>목표에 대한 기여</label><p>{priority.whyNow}</p><label>내일까지 만들 결과물</label><p className="brief-outcome">{priority.outcome}</p></div><div><label>이렇게 진행</label><ol>{priority.approach.map((step,i)=><li key={i}>{step}</li>)}</ol></div></div>
  ...
  </article>:!running&&!starting&&<div className="brief-empty"><Sparkles size={28}/><h2>내일 무엇을, 왜 먼저 해야 할까요?</h2><p>프로젝트의 목표와 오늘까지의 진척, 회의에서 남은 결정과 실제 가용 시간을 함께 검토해 한 장의 실행안을 만듭니다.</p><div><span>진척과 미결 판단</span><ArrowRight size={15}/><span>우선순위와 실행 방법</span><ArrowRight size={15}/><span>승인한 결과물</span></div></div>}
  {!!plan?.items.filter(i=>!mapped.has(i.id)).length&&<section className="brief-retained"><h3>{brief?'유지한 기존 계획':'이전에 만든 시간 계획'}</h3>{plan.items.filter(i=>!mapped.has(i.id)).map(item=><article key={item.id}><strong>{snapshot.data.tasks.find(t=>t.id===item.taskId)?.title??item.draftTask?.title??'저장된 실행 항목'}</strong><span>{formatTime(item.start)}–{formatTime(item.end)}</span>{decisionButtons(item.id)}</article>)}</section>}
