...
export function DailyBriefPanel({snapshot,date,setDate,energy,setEnergy,busy,demo,refresh,perform,navigate,open,launch}:Props){
 ...
 return <section className="daily-brief-surface">
  ...
  {plan&&<AppliedRules data={snapshot.data} plan={plan} busy={busy||running} perform={perform} onExperiments={()=>navigate('experiments')}/>}
  ...
  {plan&&!historical&&<section className="phase2-record"><div className="phase2-toolbar"><h3>일정 변경에 따른 재배치</h3><button className="secondary-button" disabled={locked} onClick={()=>void perform({type:'proposal.replan.prepare',date},'현재 시각과 최신 일정으로 대안을 계산했습니다.')}>대안 다시 계산</button></div><p>자동 실행 중에는 일정·업무 변경을 확인해 대안을 준비합니다. 승인한 일정과 보류는 유지하고 남은 업무만 배치합니다.</p>{plan.replan&&<><small>{new Date(plan.replan.generatedAt).toLocaleString('ko-KR')} 계산 · {formatTime(plan.replan.earliestStart)} 이후</small>{plan.replan.conflicts.map((c,i)=><p className="brief-alert" key={i}>{c}</p>)}<div className="phase2-records">{plan.replan.alternatives.map((p,index)=><article className="phase2-record" key={index}><h4>{index===0?'평소 계획':'부담 완화 계획'}</h4>{p.items.filter(i=>i.state==='pending').map(i=><p key={i.id}>{formatTime(i.start)}–{formatTime(i.end)} · {snapshot.data.tasks.find(t=>t.id===i.taskId)?.title??i.draftTask?.title??i.taskId}</p>)}<p>미배치 {p.unscheduled.length}건 · 남은 시간과 집중 한도를 반영</p><button className="primary-button" disabled={locked} onClick={()=>void perform({type:'proposal.replan.apply',date,basis:plan.replan!.basis,choice:index as 0|1},'미승인 계획을 바꿨습니다. 항목별 승인 후 일정에 반영됩니다.')}>이 대안으로 다시 검토</button></article>)}</div></>}</section>}
