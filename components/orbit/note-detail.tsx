...
interface Props {requestedRevision?:number;meta:Note;notes:Note[];onNote:(id:string)=>void;tasks:Task[];demo:boolean;busy:boolean;defaultDue:string;projectName?:string;onAction:(action:WorkspaceAction,message?:string)=>Promise<boolean>;onEdit:(note:Note)=>void;onDelete:()=>void;onTask:(id:string)=>void;onProject:()=>void;onNewTask:()=>void}
...
export function NoteDetail({requestedRevision,meta,notes,onNote,tasks,demo,busy,defaultDue,projectName,onAction,onEdit,onDelete,onTask,onProject,onNewTask}:Props){
 const [note,setNote]=useState<Note|null>(meta.bodyStored||(requestedRevision!==undefined&&requestedRevision!==(meta.revision??1))?null:meta),[error,setError]=useState(''),[attempt,setAttempt]=useState(0);
 ...
 // eslint-disable-next-line react-hooks/set-state-in-effect -- the body is cleared before the requested revision is fetched so stale content is not shown for the new note
 useEffect(()=>{setNote(null);if(!meta.bodyStored&&(!requestedRevision||requestedRevision===(meta.revision??1))){setNote(meta);return}const controller=new AbortController();setError('');void fetchNote<Note>({id:meta.id,revision:String(requestedRevision??meta.revision??1)},controller.signal).then(setNote).catch(error=>{if(!controller.signal.aborted)setError(error.message)});return()=>controller.abort()},[meta.id,meta.revision,meta.bodyStored,requestedRevision,attempt]);
 ...
 const historical=requestedRevision!==undefined&&requestedRevision!==(meta.revision??1);
 const linked=tasks.filter(t=>t.noteId===meta.id);
 return <>
  <div className="section-title"><button className="secondary-button" disabled={busy||historical} onClick={()=>onEdit(note)}><Pencil size={14}/>내용 수정</button><button className="text-button danger-text" disabled={busy||historical} onClick={onDelete}><Trash2 size={14}/>삭제</button></div>
  <div className="note-version"><span>{projectName??'개인 기록'}</span><span>버전 {note.revision??1} · {note.updated}</span></div>
  {!historical&&note.kind==='meeting'&&<MeetingReview noteId={note.id} revision={note.revision??1} demo={demo} onChanged={()=>window.dispatchEvent(new Event('orbit-meeting-approved'))}/>}
  {historical&&<p role="status">기록 당시 버전 {requestedRevision}입니다. 현재 버전은 {meta.revision??1}입니다.</p>}<h3>회의록 원문 · 본문</h3>{note.kind==='wiki'?<WikiDocument body={note.body} notes={notes} onOpen={onNote}/>:<div className="note-document">{note.body||'아직 작성한 본문이 없습니다.'}</div>}
  <button className="secondary-button full-width note-history-toggle" disabled={historyBusy||demo||historical} onClick={()=>historyOpen?setHistoryOpen(false):void loadHistory()}><History size={16}/>{demo?'변경 이력은 실제 기록에서 제공됩니다':historyBusy?'이력 불러오는 중':historyOpen?'변경 이력 접기':'이전 내용 확인 · 복원'}</button>
  ...
  {!historical&&note.kind==='meeting'&&<details><summary>명시된 할 일 직접 선택</summary><MeetingActions key={`${note.id}:${note.revision??1}`} note={note} tasks={tasks} busy={busy} defaultDue={defaultDue} onAction={onAction}/></details>}
