...
export function NoteDetail({requestedRevision,meta,notes,onNote,tasks,demo,busy,defaultDue,projectName,onAction,onEdit,onDelete,onTask,onProject,onNewTask}:Props){
 ...
 return <>
  ...
  {historical&&<p role="status">기록 당시 버전 {requestedRevision}입니다. 현재 버전은 {meta.revision??1}입니다.</p>}<h3>본문</h3>{note.kind==='wiki'?<WikiDocument body={note.body} notes={notes} onOpen={onNote}/>:<div className="note-document">{note.body||'아직 작성한 본문이 없습니다.'}</div>}
  ...
  {!historical&&note.kind==='meeting'&&<MeetingActions key={`${note.id}:${note.revision??1}`} note={note} tasks={tasks} busy={busy} defaultDue={defaultDue} onAction={onAction}/>}
