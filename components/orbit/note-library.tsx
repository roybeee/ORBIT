...
export function NoteLibrary({notes,kind,query,revision,demo,onSelect,onRefresh,projectId='',tag='',documentKind=''}:{notes:Note[];kind:'wiki'|'knowledge'|'all';query:string;revision:number;demo:boolean;onSelect:(id:string)=>void;onRefresh:()=>Promise<void>;projectId?:string;tag?:string;documentKind?:string}){
 ...
 },[demo,notes,kind,query,revision,page,attempt,projectId,tag,documentKind]);
 ...
 return <>{items.length===0?<div className="empty-state"><Inbox size={30}/><h3>일치하는 기록이 없습니다</h3><p>다른 검색어를 입력하거나 새로운 기록을 추가해 보세요.</p></div>:<div className="document-grid">{items.map(note=><button className="document-card" key={note.id} onClick={()=>onSelect(note.id)}>{note.kind==='knowledge'?<Lightbulb size={24}/>:note.kind==='meeting'?<FileText size={24}/>:<BookOpen size={24}/>}<h2>{note.title}</h2><p>{note.summary}</p>{query&&(note as Note&{searchExcerpt?:string}).searchExcerpt&&<blockquote className="search-excerpt">{(note as Note&{searchExcerpt?:string}).searchExcerpt}</blockquote>}<div className="doc-foot"><span>{note.tags.slice(0,2).join(' · ')}</span><span>{note.updated.slice(5).replace('-','.')}</span></div></button>)}</div>}{(page>0||more)&&<nav className="note-pagination" aria-label="기록 목록 페이지"><button className="secondary-button" disabled={page===0} onClick={()=>setPage(p=>p-1)}><ChevronLeft size={16}/>이전</button><span>{page+1}페이지</span><button className="secondary-button" disabled={!more} onClick={()=>setPage(p=>p+1)}>다음<ChevronRight size={16}/></button></nav>}</>;
}
