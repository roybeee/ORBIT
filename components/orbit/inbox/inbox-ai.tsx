...
const card=(item:AgentAction,lead?:ReactNode)=><ActionCard key={item.id} item={item} snapshot={snapshot} review={review} onAskOther={askOther} lead={lead}/>;
 return <>
  {meetings.map((noteId,index)=>{const note=snapshot.data.notes.find(n=>n.id===noteId);return <MeetingDecisions key={noteId} noteId={noteId} title={note?.title??'회의록'} items={open.filter(a=>meetingOf(a)===noteId)} onOpenNote={onOpenNote} initiallyOpen={index<5}/>;})}
