...
export function MeetingDecisions({noteId,title,items,snapshot,review,onOpenNote,onAskOther}:Props){
 ...
 async function saveDue(item:AgentAction){
  ...
  setRunning(item.id);
  try{await agentRequest('/api/meetings/review','PATCH',{noteId,actionId:item.id,due});await agentRefresh().catch(()=>{})}
  catch(error){setFailures(previous=>({...previous,[item.id]:(error as Error).message}))}
  finally{setRunning('')}
 }
 async function run(label:string,plan:ReturnType<typeof bulkPlan<AgentAction>>){
  ...
  try{
   for(const item of plan.approve)await decide(item,'approve');
   for(const item of plan.reject)await decide(item,'reject');
   for(const {item,reason} of plan.blocked)failed[item.id]=reason;
  }finally{
   setFailures(failed);setSelected(new Set());setConfirmReject(false);
 ...
 return <section className="inbox-group meeting-decisions" aria-label={`회의 결재 · ${title}`}>
    ...
    return <li key={item.id} className={'decision-row '+item.state}>
     <div className="decision-line">
      ...
      {isPending&&<div className="decision-quick">
       <button className="icon-button" aria-label={item.title+' 승인'} disabled={busy||row.needsDue} onClick={()=>void review.decide(item,'approve')}>{review.acting===item.id?<LoaderCircle size={15} className="animate-spin"/>:<Check size={15}/>}</button>
     ...
     {row.needsDue&&isPending&&<form className="decision-due" onSubmit={e=>{e.preventDefault();void saveDue(item)}}><label>마감일<input type="date" required value={dates[item.id]??''} onChange={e=>setDates(previous=>({...previous,[item.id]:e.target.value}))}/></label><button className="secondary-button" disabled={busy||!dates[item.id]}>저장</button></form>}
  ...
  {pending.length>0&&<div className="decision-bulk">
   <button className="primary-button" disabled={busy||!selected.size} onClick={()=>void run('approve',bulkPlan(items,selected,{rejectRest:false}))}>{running==='approve'?<LoaderCircle size={15} className="animate-spin"/>:<Check size={15}/>} 선택 {selected.size}건 승인</button>
   <button className={'secondary-button'+(confirmReject?' decision-reject-confirm':'')} disabled={busy||!rest.length} onClick={()=>{if(!confirmReject){setConfirmReject(true);return}void run('reject',{approve:[],reject:rest,blocked:[]})}}>{confirmReject?`한 번 더 누르면 ${rest.length}건 반려`:`선택하지 않은 ${rest.length}건 반려`}</button>
