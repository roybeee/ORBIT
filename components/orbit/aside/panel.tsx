...
export function AsidePanel({snapshot,perform,busy,demo,onAsk,visible}:{snapshot:WorkspaceSnapshot;perform:(action:WorkspaceAction,message?:string)=>Promise<boolean>;busy:boolean;demo:boolean;onAsk:(text:string)=>void;visible:boolean}){
  ...
  async function confirmResult(job:AsideJob){
    if(!health||health.bridgeId!==job.bridgeId||!job.result.trim()){toast.error('이 PC에서 종료 상태와 결과를 먼저 확인해 주세요.');return;}
    if(!window.confirm('ASIDE에서 작업이 종료되었고 아래에 반환된 결과가 맞는지 확인했나요? 확인하면 Hermes가 이 결과로 업무를 이어갑니다.'))return;
    setWorking(true);try{const run=await local<AsideRun>(token,'/acknowledge',{runId:job.runId,confirmed:true});await server({action:'confirm_result',id:job.id,bridgeId:job.bridgeId,runId:job.runId,seq:run.seq,result:run.result||job.result});await refresh()}catch(e){toast.error((e as Error).message)}finally{setWorking(false)}
  }
