...
export function EventReviewCard({review,busy,onResolve}:{review:EventReview;busy:boolean;onResolve:(action:Extract<WorkspaceAction,{type:'event.review'}>)=>void}){
 ...
 return <article className="event-review-card" aria-label={`${review.title} 일정 검토`}>
  ...
  <div className="event-review-options">
   <button disabled={busy} className="secondary-button" onClick={()=>onResolve({type:'event.review',reviewId:review.id,decision:'complete'})}>완료만 표시</button>
   <form onSubmit={event=>{event.preventDefault();if(title.trim()&&due)onResolve({type:'event.review',reviewId:review.id,decision:'next',title:title.trim(),due})}}>
   ...
   <form onSubmit={event=>{event.preventDefault();if(followUpAt)onResolve({type:'event.review',reviewId:review.id,decision:'defer',followUpAt:new Date(followUpAt).toISOString()})}}>
