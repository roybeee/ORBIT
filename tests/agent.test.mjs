...
test('live Google conflicts prevent event creation while planning uses newly synchronized busy periods',()=>fixture(async db=>{
 await connect(db);let posts=0;globalThis.fetch=async(url,options={})=>{if(options.method==='POST'){posts++;throw new Error('must not create')}return url.includes('/events/')?j({},404):j({items:[{id:'busy',start:{dateTime:tomorrow+'T09:00:00+09:00'},end:{dateTime:tomorrow+'T10:00:00+09:00'}}]})};await assert.rejects(()=>createGoogleEvent(db,'owner',env,randomUUID(),googleAction),e=>e.code==='CALENDAR_OVERLAP');assert.equal(posts,0);const [card]=await stage(db,[{type:'proposal.generate',date:tomorrow,energy:'normal'}]);await decide(db,'owner',{id:card.id,decision:'approve'},env);const refreshed=await readWorkspace(db,'owner');assert.equal(refreshed.data.proposals.length,1);assert.ok(refreshed.data.events.some(e=>e.id==='google:busy:'+tomorrow));
}));
...
test('target changes during a native run automatically revalidate without publishing stale cards',()=>fixture(async db=>{
 await connectHermes(db);const input={id:randomUUID(),message:'프로젝트'};await runAgent(db,'owner',input,env);globalThis.fetch=async(url,options)=>options.method==='POST'?j({run_id:'run_1',status:'started'},202):completed(final([{type:'project.upsert',project}]));await advanceAgent(db,'owner',input.id,env);await writeCommand(db,'owner',{operationId:randomUUID(),expectedRevision:0,action:{type:'project.upsert',project:{...project,name:'새로운 결과물'}}});await advanceAgent(db,'owner',input.id,env);assert.equal((await listAgent(db,'owner')).turns[0].status,'running');assert.match((await listAgent(db,'owner')).turns[0].progress,/자동/);assert.equal((await listAgent(db,'owner')).actions.length,0);assert.equal((await readWorkspace(db,'owner')).data.projects[0].name,'새로운 결과물');
}));
