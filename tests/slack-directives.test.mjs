...
// Deliberately a past date: a directive filed for any day but the server's today must still read
// back as completed. note.upsert stamps its own today, which readback must not compare against.
...
test('real canonical note and atomic receipt replay/readback',async()=>{assert.ok(await import(moduleUrl).catch(()=>null),'canonical Slack backend must exist');const s=await setup();try{const first=await call(s,'',wire());assert.equal(first.status,200,JSON.stringify(first.data));assert.equal(first.data.status,'completed');const note=await readNote(s.db,'owner',first.data.target.id);assert.equal(note.kind,'knowledge');assert.equal(note.body,wire().change.text);assert.equal((await call(s,'',wire())).data.id,first.data.id);assert.equal((await readWorkspace(s.db,'owner')).data.notes.length,1);assert.equal((await call(s,'?operationKey=origin%3Atest')).data.target.note.body,note.body);assert.equal((await call(s,'?operationKey=origin%3Atest')).data.status,'completed','a filing date other than the server today is not an edit');}finally{s.db.close()}});
