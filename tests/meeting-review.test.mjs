...
test('Plaud import queues only ready source, idempotently; long split sources each get a review',async()=>{const db=createDatabase();try{const record={id:'plaud-test',title:'녹음',started:'2026-09-22',duration:10,transcript:'원문',summary:'',pending:true};await importRecording(db,owner,record);assert.equal((await db.prepare('SELECT count(*) AS n FROM orbit_meeting_reviews').first()).n,0);await importRecording(db,owner,{...record,transcript:'가'.repeat(190000),summary:'회의',pending:false});const n=(await db.prepare('SELECT count(*) AS n FROM orbit_meeting_reviews').first()).n;assert.equal(n,3);await importRecording(db,owner,{...record,transcript:'가'.repeat(190000),summary:'회의',pending:false});assert.equal((await db.prepare('SELECT count(*) AS n FROM orbit_meeting_reviews').first()).n,n)}finally{db.close()}});
