...
// Install before the readiness marker so shutdown tests exercise a live CLI.
if(prompt.includes('controlled shutdown'))process.on('SIGTERM',()=>{
 appendFileSync(file+'.stopping','stop\n');
 setInterval(()=>{if(existsSync(file+'.release'))process.exit(0)},10);
});
